#-----------------------------------------------------------------
# CURVE_WEIB: Weibull curve
#-----------------------------------------------------------------

WEIB.get_mu <- function(par, times, options=list())
{
       
	return ( par[[1]]*(1-exp(-par[[2]]*times^par[[3]])) );
	
}


WEIB.get_init_rand<-function(dat, times)
{
	...
	par.a <- um * um / um.1;
	par.b <- -log(u11/par.a-1);           ########
	par.c <- try( -(log((log(um/par.a-1))/(-par.b)))/log(m) );
...
WEIB.get_summary <-function( curve, format)
{
...
class_WEIB<-list(
	...
	par_name 	= c( "a", "b", "c"),
	get_mu      	= WEIB.get_mu,
	get_summary    	= WEIB.get_summary,
	get_init_rand   = WEIB.get_init_rand);
...
ZZZ.regcovar_weib<-function()
{
	CURVE_WEIB <<- FM2.reg_curve( class_WEIB );
}
