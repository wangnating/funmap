#-----------------------------------------------------------------
# CURVE_HOSS:  Hossfeld IV Curve
#-----------------------------------------------------------------

HOSS.get_mu <- function(par, times, options=list())
{
       
	return ( times^par[[3]]/(par[[2]]+times^par[[3]]/par[[1]]) );
	
}


HOSS.get_init_rand<-function(dat, times)
{
	...
	par.a <- um * um / um.1;
	par.b <- (par.a-u11)/(u11*par.a);
	par.r <- try( log((um*par.a*par.b)/(par.a-um))/(log(m)) );
...
class_HOSS<-list(
	type 	 	= 6,
	name 	 	= "HOSS",
	desc 	 	= "Hossfeld IV Curve",
	par_num    	= 3, 
	par_name 	= c( "a", "b", "c"),
	get_mu      	= HOSS.get_mu,
	get_summary    	= HOSS.get_summary,
	get_init_rand   = HOSS.get_init_rand);
...
ZZZ.regcovar_hoss<-function()
{
	CURVE_HOSS <<- FM2.reg_curve( class_HOSS );
}