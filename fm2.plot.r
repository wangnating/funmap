...
#    7) plot_correlation
#    8) plot_eval_ret
#    9) plot_genetic mapping
#    10)plot_genotype mapping
# 
# History:
...
fpt.plot_overlapping_curves<-function( dat, options=list() )
{
	...
	if (!is.null(dat$QQ_par))
	{
		...
		y10  <- FM.curve$get_mu(dat$QQ_par, x10, options=list(tmin=1, tmax=xmax) );
	...
	if (!is.null(dat$curve))
	{
		x10 <- seq(xmin,xmax,0.1);	
		y10  <- FM2.curve$get_mu( dat$curve$par, x10, options=list(tmin=1, tmax=xmax) );
		lines(x10, y10, lwd=1, col="black" , lty="solid");	
	}
...
{
	chrs <- c();
	for(i in 1:max(dat$marker_table[,3]))
	{
		chrs[i] <- max(as.numeric(dat$marker_obj$grps[[i]]$dists));
	}
	
	r <- try( optimize( is.fit, lower= sum(chrs)/12, upper= sum(chrs)/5 ,chrs=chrs),F);
	y.lim <- r$minimum;
	
	return(y.lim)
}

marker.plot.idx_count <- function( dat, y.lim )
{
	marker <- dat$marker_table;
	markerobj <- dat$marker_obj;
	I=1;
	I.m <- c();
	idx <- c();
	idx[1] <- 1;
	...
	for(i in 1:max( marker[,3]))
	{
		chrs[i] <- max(as.numeric(markerobj$grps[[i]]$dists));
		if ( y.start+chrs[i] > y.lim )
		{
			y.start <- 0 ;
			I=1;
			Dis[i] <- chrs[i];
	...
	return(list( idx = idx ,idx_count = max(idx), DIS = Dis, rep.max=max(I.m) ));
}

chro_start.top <- function( marker_table, marker_obj, TOP,chr )
{
	idx <- chr$idx;
	DIS <- chr$DIS;
	TOP <- TOP;
	name_h <- strheight( marker_obj$grps[[1]]$names[[1]]);
	
	start.top <- c();
	start.top[1] <- TOP;
	
	for(i in 2:max(marker_table[,3]) )
	{
		if( idx[i]==idx[i-1] )
			start.top[i] <- start.top[i-1]-DIS[i-1]-name_h*3
		else
		start.top[i] <- TOP;
	}
	return(start.top)
...
fpt_plot_genotype <- function( dat )
{ 
	pdf(file="genotype.pdf",width=10,height=8) 
	marker <- dat$marker_table;
	markerobj <- dat$marker_obj;
	y.lim <- y.lim( dat);
	chr <- marker.plot.idx_count(dat,y.lim);
	TOP <- y.lim+chr$rep.max*(y.lim/25);
	plot(c(5,90), c(0, TOP+10),type="n",cex=0,axes=FALSE,xlab="",ylab="");
	idx <- chr$idx;
	width <- 10;
	start.top <- chro_start.top( marker, markerobj,TOP, chr );
	
	for(i in 1:max( marker[,3]))
	{
		marker.dis <- as.numeric(markerobj$grps[[i]]$dists) ;
		marker.name <- markerobj$grps[[i]]$names;
		name_h<- strheight(marker.name, cex=0.5);
		count <- markerobj$grps[[i]]$count; 
		bar.width <- 0.3;
		rect( idx[i]*width, start.top[i]-chr$DIS[i], idx[i]*width+bar.width, start.top[i], col="white" );
		text( idx[i]*width, start.top[i], pos=3,labels=markerobj$grps[[i]]$id, adj=c(0.5,0.5),cex=0.8,col="red");
	    	even <- seq(2, count, by = 2);
	    	
	    	####j=1
	    	segments(idx[i]*width-0.2, start.top[i]-marker.dis[1], idx[i]*width+bar.width+0.2, start.top[i]-marker.dis[1], lwd=0.5);
		text( idx[i]*width+bar.width+0.5, start.top[i]-marker.dis[1], labels=sprintf( "%4s",marker.name[[1]]), adj=0, cex=0.5, font=2 );
		
		dis.idx <- marker.dis;
		for(j in 2:count)
		{
			segments(idx[i]*width-0.2, start.top[i]-marker.dis[j], idx[i]*width+bar.width+0.2, start.top[i]-marker.dis[j], lwd=0.7);
			if( name_h[j]> dis.idx[j]-dis.idx[j-1])
			{
				dis.idx[j] <- dis.idx[j-1];
			}
			else
			{
				dis.idx[j] <- dis.idx[j];
				text( idx[i]*width+bar.width+0.5, start.top[i]-marker.dis[j], labels=sprintf( "%4s",marker.name[[j]]), adj=0, cex=0.5, font=2 );
				text( idx[i]*width-0.5, start.top[i]-( marker.dis[j]+marker.dis[j-1])/2,
						labels=sprintf("%5.1f",mean( marker.dis[j]-marker.dis[j-1] )),adj=1, cex=0.5 );				
			}
			for(j in even )
				rect(idx[i]*width , start.top[i]-marker.dis[j], idx[i]*width+bar.width, start.top[i]-marker.dis[j-1], col="black" );
		}			
	}
	dev.off()
}
...
#     dat :
#--------------------------------------------------------------
fm.genetic.map <- function (dat)
{
	pdf(file="genetic.pdf",width=10,height=5) 
	genos_table <- dat$genos_table;
	marker_table <- dat$marker_table;
	len.grps <- length(dat$marker_obj$grps);
	len.marker <- dim( genos_table)[2];
	len.sample <- dim(genos_table)[1];
	
	xlim <- len.marker + len.grps*3;
	par(mai=c(mai=c(0.5,0.5,0.5,0.5)));
	plot(c(0,xlim),c(-5,len.sample+10),type="n",cex=0,axes=FALSE,xlab="",ylab="");
	
	marker.idx <- 1;
	for(i in 1:len.grps)
	{
		lenmar <- dat$marker_obj$grps[[i]]$count;
		geno <- genos_table[,marker.idx:(marker.idx+lenmar-1) ];
		for(j in 1:len.sample)
		{	
			col <- rep("yellow",len.marker);
			green2 <- which(geno[j,]==2);
			col[green2] <- "green";
			red1 <- which(geno[j,]==1);
			col[red1] <- "red";
			blue0 <- which(geno[j,]==0);
			col[blue0] <- "blue";
			m <- 0:(lenmar - 1)+marker.idx+(i-1)*3;
			rect(m,j,m+1,j+1,col = col,border="gray55");
		}
		text( marker.idx+(i-1)*3+(lenmar-1)/2,0, labels=dat$marker_obj$grps[[i]]$id ,adj=c(1,1), cex=0.7,srt=90,font=2,lwd=0.001);
		marker.idx <- marker.idx+lenmar;	
	}
	rect(xlim-72,len.sample+3, xlim-2, len.sample+8);
	rect(xlim-70,len.sample+4, xlim-65,len.sample+6,col="green");
	text(xlim-61,len.sample+5,"AA", cex=0.7);
	rect(xlim-55,len.sample+4, xlim-50,len.sample+6,col="red");
	text(xlim-46,len.sample+5,"Aa", cex=0.7);
	rect(xlim-40,len.sample+4, xlim-35, len.sample+6,col="blue");
	text(xlim-31,len.sample+5,"aa", cex=0.7);
	rect(xlim-25,len.sample+4, xlim-20, len.sample+6,col="yellow");
	text(xlim-10,len.sample+5, "missing", cex=0.7);
	dev.off()
}