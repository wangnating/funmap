...
#
#--------------------------------------------------------------
FM2.qtlscan<-function( dat_obj, options=list() )
{
...
#
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
FM2.one_step<-function(pheno_csv, geno_csv, marker_csv, curve_type, cross_type, covar_type, options=list(prob=0.05))
{
	if (is.null( FM_sys ) )
		FM2.start();
...
FM2.simu_test<-function(cross_type, par_obj=par_LC, curve_type=CURVE_LC, covar_type =COVAR_AR1 )
{
	...
	est <- FM2.data_est(dat_st, cross_type);
	if (is.null(est))
	...
	summary(dat_st);
	browser()
	ret_st<- FM2.qtlscan( dat_st);
	browser()
	summary(ret_st, dat_st);
     
	##Step 1
	rdata.name <- paste( "simu_test_", FM2.curve$name, "_", FM2.covar$name, "_", FM2.cross$name,".rdata", sep="");
	save(dat_st, ret_st, file = rdata.name );

	##Step 2(Report) 
	pdf.name <- paste( "simu_test_", FM2.curve$name, "_", FM2.covar$name, "_", FM2.cross$name,".pdf", sep="");
